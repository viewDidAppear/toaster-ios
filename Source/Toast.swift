...
class Toast: UIView {
    ...
    private func getStyle() -> CGRect {
        ...
        switch style! {
            case .Brief:
                rect.size.height = round(screenSize.height/14.0)
        }
    ...
    private func getIcon() -> UIImage? {
        switch toastIcon! {
        case .Bang:
            return UIImage(named: "triangle_bang.png")?.imageWithRenderingMode(.AlwaysTemplate)
        case .General:
            return UIImage(named: "toast_generic.png")?.imageWithRenderingMode(.AlwaysTemplate)
        case .Intero:
            return UIImage(named: "toast_generic.png")?.imageWithRenderingMode(.AlwaysTemplate)
        }
    }
    ...
    func hide() {
        ...
        if animated == true {
            UIView.animateWithDuration(0.3, delay: 0.0, options: .CurveEaseInOut, animations: { [weak self] in
                guard let weakSelf = self else { fatalError("No self!") }
                weakSelf.frame.origin.y = -(weakSelf.frame.size.height * 3)
                }, completion: { [weak self] finished in
